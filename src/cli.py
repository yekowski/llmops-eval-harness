...
async def evaluate_dataset(
    ...
    # Strict provenance tracking: Fail closed if judge provenance is local, mock, fallback, or unknown
    uncertified_judge_count = sum(
        1 for r in results
        if r.judge_provenance != "remote_llm"
        or (r.retrieval_judge_provenance is not None and r.retrieval_judge_provenance != "remote_llm")
    )
    fallback_generation_count = sum(1 for r in results if r.judge_mode in ["fallback", "mock"] or r.judge_provenance != "remote_llm")
    fallback_retrieval_count = sum(1 for r in results if (r.retrieval_judge_mode and (r.retrieval_judge_mode in ["fallback", "mock"] or r.retrieval_judge_provenance != "remote_llm")))
    has_fallback = (fallback_generation_count > 0) or (fallback_retrieval_count > 0) or (uncertified_judge_count > 0)
    ...
    metrics = {
        ...
        "uncertified_judge_count": uncertified_judge_count,
...
def _evaluate_sla_gates(metrics: dict, thresholds: dict, judge_failure_policy: str = "fail") -> List[str]:
    ...
    # 1. Fallback & Judge Provenance Policy Check (P0 Safety: Fail closed on uncertified judge provenance)
    if metrics.get("has_fallback", False) or metrics.get("uncertified_judge_count", 0) > 0:
        uncertified = metrics.get("uncertified_judge_count", 0)
        gen_fb = metrics.get("fallback_generation_count", 0)
        ret_fb = metrics.get("fallback_retrieval_count", 0)
        msg = f"Evaluation relied on uncertified/fallback judge grading ({uncertified} uncertified provenance, {gen_fb} gen, {ret_fb} retrieval fallbacks)."

        if judge_failure_policy == "fail":
            failures.append(
                f"Judge failure policy is 'fail' and {msg} Local/mock/fallback/unknown judge provenance must fail closed in strict CI mode."
            )
...
async def main_async(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        print(f"Fatal error loading configuration or dataset: {e}", file=sys.stderr)
        return 1
    ...
    return 0 if sla_passed else 1


def main(argv: Optional[List[str]] = None) -> int:
    """CLI main entrypoint returning an integer exit code (0 for pass, 1 for fail)."""
